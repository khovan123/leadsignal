...
async function handleMessage(message) {
  switch (message?.type) {
    ...
    case 'GET_STATE':
      return { ok: true, state: await getState(), config: await lsGetConfig() };
    case 'SAVE_CONFIG':
      return { ok: true, config: await lsSaveConfig(message.payload || {}) };
    case 'CAPTURE_CURRENT_PAGE':
      return captureCurrentPage(Number(message.payload?.limit || 50));
    default:
...
async function pairDevice(payload) {
  ...
  const result = await lsApiFetch(config, '/auth/extension/pair', {
    ...
    body: JSON.stringify({
      ...
      displayName: payload.displayName ? String(payload.displayName).slice(0, 100) : undefined,
    }),
  });
...
async function captureCurrentPage(limit) {
  ...
  const response = await chrome.tabs.sendMessage(tab.id, {
    ...
    limit: Math.max(1, Math.min(Math.round(limit || 50), 100)),
  });
  if (!response?.ok || !response.batch?.posts?.length) {
    throw new Error(response?.error || 'No rendered Reddit posts were found');
  }
  const timestamp = new Date().toISOString();
  const nonce = crypto.randomUUID();
  const proof = await lsSignText(
    `LeadSignal extension ingestion v1\n${timestamp}\n${nonce}\n${lsStableStringify(response.batch)}`,
  );
  const config = await lsGetConfig();
  const result = await lsApiFetch(config, '/extension/ingest', {
    method: 'POST',
    body: JSON.stringify({ deviceId, timestamp, nonce, proof, batch: response.batch }),
  });
