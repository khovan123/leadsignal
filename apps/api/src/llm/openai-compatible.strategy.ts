...
export class OpenAiCompatibleStrategy implements LlmStrategy {
  supports(provider: LlmProvider) {
    return [LlmProvider.OPENAI, LlmProvider.OPENROUTER, LlmProvider.GITHUB_MODELS, LlmProvider.CUSTOM_OPENAI_COMPATIBLE].includes(provider);
  }
  private base(connection: StrategyConnection) {
    if (connection.baseUrl) return connection.baseUrl.replace(/\/$/, '');
    if (connection.provider === LlmProvider.OPENROUTER) return 'https://openrouter.ai/api';
    if (connection.provider === LlmProvider.GITHUB_MODELS) return 'https://models.inference.ai.azure.com';
    return 'https://api.openai.com';
  }
  async verify(connection: StrategyConnection, model = 'gpt-4o-mini') {
  ...
  async execute(connection: StrategyConnection, request: StrategyRequest): Promise<StrategyResult> {
    ...
    const response = await fetchWithTimeout(`${this.base(connection)}/v1/chat/completions`, {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `Subreddit: ${request.subreddit}\nTitle: ${request.title}\nBody: ${request.body}` },
        ],
