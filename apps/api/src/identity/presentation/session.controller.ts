...
export class SessionController {
  ...
  @Post('register') register(@Body() body:RegisterInput,@Headers('user-agent') userAgent?:string,@Ip() ip?:string){return this.commands.execute(new RegisterUserCommand(body,{userAgent,ip}));}
  @Post('login') login(@Body() body:LoginInput,@Headers('user-agent') userAgent?:string,@Ip() ip?:string){return this.commands.execute(new LoginUserCommand(body,{userAgent,ip}));}
