...
import type { LeadRecord, LeadRepository } from '../domain/lead.repository';
...
export class PrismaLeadRepository implements LeadRepository {
  ...
  async exists(workspaceId: string, leadId: string): Promise<boolean> {
    const count = await this.prisma.lead.count({ where: { id: leadId, workspaceId } });
    return count > 0;
  }

  updateStatus(workspaceId: string, leadId: string, status: LeadStatusValue): Promise<LeadRecord> {
