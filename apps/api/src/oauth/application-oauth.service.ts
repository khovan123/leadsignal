...
export class ApplicationOAuthService {
  constructor(private readonly prisma: PrismaService, private readonly states: OAuthStateService, private readonly broker: OAuthBrokerService, private readonly profiles: ProviderProfileService) {}
  ...
  async callback(provider: string, query: Record<string, unknown>) {
    ...
    return { user, workspaceId: membership.workspaceId, redirectUrl: this.redirect(state.metadata) };
  }
